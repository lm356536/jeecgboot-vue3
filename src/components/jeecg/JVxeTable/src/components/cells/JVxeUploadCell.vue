  ...
  <div>
    <template v-if="hasFile" v-for="(file, fileKey) of [innerFile || {}]" :key="fileKey">
      <a-input :readOnly="true" :value="file.name">
        ...
        <span v-if="file.status === 'uploading'" slot="addonAfter"
          >{{ Math.floor(file.percent) }}%</span
        >
        <template
          v-if="originColumn.allowDownload !== false || originColumn.allowRemove !== false"
          #addonAfter
        >
          <Dropdown :trigger="['click']" placement="bottomRight">
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item
                  v-if="originColumn.allowDownload !== false"
...
<script lang="ts">
  ...
  import {
    ...
    fileGetValue,
    fileSetValue,
  } from '../../hooks/cells/useJVxeUploadCell';
