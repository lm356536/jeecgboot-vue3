...
export function useMethods(
  ...
  function handleActiveMethod({ row, column }) {
    const flag = (() => {
      ...
      // 执行增强
      return (
        getEnhanced(column.params.type).aopEvents.activeMethod!.apply(instance, arguments as any) ??
        true
      );
    })();
    if (!flag) {
  ...
  async function addOrInsert(
    rows: Recordable | Recordable[] = {},
    index,
    triggerName,
    options?: IAddRowsOptions,
  ) {
    const xTable = getXTable();
    let records;
    if (isArray(rows)) {
  ...
  async function pushRows(
    rows: Recordable | Recordable[] = {},
    options = { setActive: false, index: -1 },
  ) {
    const xTable = getXTable();
  ...
  async function clearSelection() {
    const xTable = getXTable();
    const event = { $table: xTable, target: instance };
