...
export function usePopBiz(props, tableRef?) {
  ...
  function handleSumColumn(metaColumnList: OnlineColumn[], dataTotal: number): void {
    ...
    if (pagination.isTotal) {
      ...
      pagination.total =
        Number(dataTotal) + Number(Math.floor(dataTotal / pagination.realPageSize));
    }
  }
   ...
   * 处理列的href和字典翻译
   */
  function handleColumnHrefAndDict(
    columns: OnlineColumn[],
    fieldHrefSlotKeysMap: {},
  ): OnlineColumn[] {
    for (const column of columns) {
  ...
  function onShowTotal(total) {
    ...
    const end =
      start + (pagination.isTotal ? dataSource.value.length - 1 : dataSource.value.length) - 1;
  ...
  function openHrefCompModal(href) {
    ...
    hrefComponent.value.is = defineAsyncComponent(
      () => import(/* @vite-ignore */ '/@/views/' + (path.startsWith('/') ? path.slice(1) : path)),
    );
  }
