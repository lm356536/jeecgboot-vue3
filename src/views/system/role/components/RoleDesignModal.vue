<template>
  ...
  >
    ...
    <template v-for="(item, index) of processTypeDictOptions">
      <a-card
        :title="item.text"
        :style="{ marginTop: index == 0 ? '0px' : '12px', height: 'auto' }"
        :headStyle="{ backgroundColor: '#eaeaea' }"
      >
        <a-checkbox-group
          @change="designNameChange"
          v-model:value="designNameValue[index]"
          style="width: 100%"
        >
          <a-row>
...
<script lang="ts" setup>
  ...
  async function initData() {
    ...
    if (designList) {
      ...
      //工单类型字典项
      processTypeDictOptions.value = processTypeDict.value.filter(
        (item) => procTypeArr.indexOf(item.value) != -1,
      );
      //工单名称集合
  ...
  async function handleSubmit() {
    try {
      ...
      //提交表单
      await saveRoleDesign({
        roleId: unref(roleId),
        newDesignId: unref(newDesignId),
        oldDessignId: unref(oldDesignId),
      });
      //关闭弹窗
      closeModal();
      //刷新列表
